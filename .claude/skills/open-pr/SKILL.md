...
**e2e gotcha**: `playwright.config.ts`'s default `baseURL` is the deployed Vercel URL (`https://verdict-gamma-ten.vercel.app`), not localhost. Running `pnpm test:e2e` bare tests the *live deployed site*, not your local changes. To actually test local changes:
